...
for ind in range(lenOfThresholds):
    ...
    topfile = open(str(individualThresholds[ind])+"_topPRSids.txt", 'w')
    bottomfile = open(str(individualThresholds[ind])+"_bottomPRSids.txt", 'w')
        ...
        try:
            ...
            if float(COVdf.loc[numRow]['PHENO']) == 1.0:
                if control_counter_bottom <= individualThresholds[ind]//2: ##check to make sure control to case ratio isn't overshot
                    ...
                    bottomfile.write(str(row_string))
...
#edit: you also now have two more files named bottomthreshold_PRS.txt and topthreshold_PRS.txt with the top and bottom thresholds respectively
...
for length in range(lenOfThresholds): #index for threshold list
    ...
    skat.write(
        "/projects/b1049/genetics_programs/Rvtest/rvtests/executable/rvtest --inVcf " + iVCF + " --pheno " + str(individualThresholds[length])+"_PRSids.txt --pheno-name PHENO --covar "+ str(individualThresholds[length])+"_PRSids.txt --out "+ str(individualThresholds[length])+"_PRS --covar-name " + covariateNames + " --kernel skato" 
    )
    skat.close()

    script_log.write(
        "PRS_"+str(individualThresholds[length])+"=($(sbatch "+str(individualThresholds[length])+"_SkatO.sh))\n"
        "echo "'"PRS_'+str(individualThresholds[length])+' ${PRS_' +str(individualThresholds[length])+'[-1]}"'" >> PRS_ids\n"
        "\n"
    )

    topskat = open(str(individualThresholds[length])+"_topSkatO.sh", 'w')
    sbatchWritingMid(topskat)
    topskat.write(
        "/projects/b1049/genetics_programs/Rvtest/rvtests/executable/rvtest --inVcf " + iVCF + " --pheno " + str(individualThresholds[length])+"_topPRSids.txt --pheno-name PHENO --covar "+ str(individualThresholds[length])+"_topPRSids.txt --out "+ str(individualThresholds[length])+"_topPRS --covar-name " + covariateNames + " --kernel skato" 
    )
    topskat.close()

    script_log.write(
        "PRStop_"+str(individualThresholds[length])+"=($(sbatch "+str(individualThresholds[length])+"_topSkatO.sh))\n"
        "echo "'"PRStop_'+str(individualThresholds[length])+' ${PRStop_' +str(individualThresholds[length])+'[-1]}"'" >> PRS_ids\n"
        "\n"
    )

    bottomskat = open(str(individualThresholds[length])+"_bottomSkatO.sh", 'w')
    sbatchWritingMid(topskat)
    bottomskat.write(
        "/projects/b1049/genetics_programs/Rvtest/rvtests/executable/rvtest --inVcf " + iVCF + " --pheno " + str(individualThresholds[length])+"_bottomPRSids.txt --pheno-name PHENO --covar "+ str(individualThresholds[length])+"_bottomPRSids.txt --out "+ str(individualThresholds[length])+"_bottomPRS.sh --covar-name " + covariateNames + " --kernel skato" 
    )

    script_log.write(
        "PRSbottom_"+str(individualThresholds[length])+"=($(sbatch "+str(individualThresholds[length])+"_bottomSkatO.sh))\n"
        "echo "'"PRSbottom_'+str(individualThresholds[length])+' ${PRSbottom_' +str(individualThresholds[length])+'[-1]}"'" >> PRS_ids\n"
        "\n"
    )
